...
const Demographics: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch age data
        await axios.get(url + 'patients/demo/age')
          .then(response => {
            const tempAgeData = [
              { name: '<18', value: response.data["<18"] },
              { name: '18-25', value: response.data["18-25"] },
              { name: '26-35', value: response.data["26-35"] },
              { name: '36-45', value: response.data["36-45"] },
              { name: '46-60', value: response.data["46-60"] },
              { name: '60+', value: response.data["60+"] },
            ];
            setAgeData(tempAgeData);
          });
  
        // Fetch race data
        await axios.get(url + 'patients/demo/race')
          .then(response => {
            const tempRaceData = Object.keys(response.data).map((key) => ({
              name: key,
        ...
        // Fetch gender data
        await axios.get(url + 'patients/demo/gender')
          .then(response => {
            const tempGenderData = Object.keys(response.data).map((key) => ({
              name: key,
              value: response.data[key]
            }));
        ...
        // Fetch income data
        await axios.get(url + 'patients/demo/income')
          .then(response => {
            const tempIncomeData = Object.keys(response.data).map((key) => ({
              name: key,
              value: response.data[key]
            }));
